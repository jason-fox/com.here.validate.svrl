...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  >
		...
		<xsl:if
      test="matches($running-text,$all-case-checked-words-regex,'i') and not(matches($running-text,$all-case-checked-words-regex))"
    >
			...
			<xsl:for-each select="$case-checked-words">
				...
				<xsl:if
          test="matches($running-text,$case-checked-word-regex,'i') and not(matches($running-text,$case-checked-word-regex)) and not(matches($running-text,$case-checked-word-prefixed,'i')) and not(matches($running-text,$case-checked-word-postfixed,'i'))"
        >
				<xsl:apply-templates mode="failed-assert-with-node" select="$running-text-node">
						<xsl:with-param name="rule-id">incorrect-capitalization</xsl:with-param>
						<xsl:with-param
              name="test"
            >matches($running-text,$case-checked-word-regex,'i') and not(matches($running-text,$case-checked-word-regex))</xsl:with-param>
						<!--  Placeholders -->
						<xsl:with-param name="param1" select="string(.)"/>
						<xsl:with-param name="param2" select="replace($running-text, '\\', '\\\\')"/>
					</xsl:apply-templates>
